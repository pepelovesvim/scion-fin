...
class Module {
    constructor({ code, setupFn, context, name, version, license, description, changeLog, sourceFile, previousVersion, vm, executionTimeout = 30 }) {
        this.signer = null;
        this.signature = null;
        this.vm = null;
        this.setupFn = null;
        this.cleanableScripts = [];
        if (code && setupFn) {
        ...
        if (code) {
            ...
            if (signatureResults) {
                ...
                const hash = ethers_1.ethers.utils.keccak256(ethers_1.ethers.utils.toUtf8Bytes(signedCode));
                const message = `CryptoStats Adapter Hash: ${hash}`;
                const verifiedSigner = ethers_1.ethers.utils.verifyMessage(message, signature);
